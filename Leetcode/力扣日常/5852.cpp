...
public:
	...
	int n, m;
	bool mp[70][70];
	void dfs(int curRow, int curSum, vector<vector<int>>& mat, int target){
		if(curRow == n) 
			if(abs(target - curSum) < minD){
				minD = abs(target - curSum);
				return;
			}else{
				return;
			} 
		if(curSum > target && (curSum - target) > minD) return;
		for(int i = 0; i < m ; i++){
			if(curSum + mat[curRow][i] > target && (curSum + mat[curRow][i] - target) > minD) break;
			if(i != 0 && mat[curRow][i] == mat[curRow][i-1]) continue;

			dfs(curRow + 1, curSum + mat[curRow][i], mat, target);
		}
	}
    int minimizeTheDifference(vector<vector<int>>& mat, int target) {
    	...
    	for(auto ma : mat) sort(ma.begin(), ma.end());
    	dfs(0,  0, mat, target);
    	return minD;
    }
};
int main(int argc, char * argv[]){
	...
	cout << s.minimizeTheDifference(mat, 145);
	/* code */
